...
cat <- data.frame(cbind(larger$sma_meter*(1-larger$ecc)/1000 - earth_radius,
                        larger$sma_meter*(1+larger$ecc)/1000 - earth_radius,
                        larger$inc_deg));
names(cat) <- c("peri_km","apo_km","inc_deg");
...
with(cat, plot(inc_deg, peri_km, col = "blue", pch = 1))
with(cat, points(inc_deg, apo_km, col = "red", pch = 1))
with(sc, points(inc_deg, peri_km, col = "black", pch = 20))

g <- ggplot( cat, aes( inc_deg, y = value, color = variable))
g + geom_point( aes(y = peri_km, col="red",alpha=0.5) )  +
    geom_point( aes(y = apo_km, col="blue",alpha=0.5) ) +
    labs(x = "inclination [deg]") +
    labs(y = "pericenter/apocenter altitude [km]")
```
...
orbit <- cbind("",orbit)
write.table(orbit, file = "orbit_test.txt", row.name = FALSE, sep = "\t")
```